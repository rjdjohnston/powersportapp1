...
export default function Home() {
  return (
    <main className="min-h-screen bg-[#f4f2ee] text-[#17191a]">
      <div className="bg-[#a71920] px-6 py-2 text-center text-[11px] font-semibold uppercase tracking-[0.22em] text-white sm:text-right sm:px-10">
        Talk to an appraiser <a className="ml-2 underline underline-offset-4" href="tel:3054842442">305.484.2442</a>
      </div>
      <header className="absolute left-0 right-0 top-8 z-10 border-b border-white/15 bg-[#111516]/90 text-white backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 lg:px-10">
          <a href="#top" className="flex items-center gap-3" aria-label="Powersport Appraisals home">
            <span className="flex h-10 w-10 items-center justify-center border border-[#d7a23b] text-lg font-black text-[#d7a23b]">PA</span>
            <span className="leading-none"><strong className="block text-sm tracking-[0.2em]">POWERSPORT</strong><small className="block pt-1 text-[9px] tracking-[0.38em] text-[#d7a23b]">APPRAISALS</small></span>
          </a>
      ...
      <section id="about" className="mx-auto grid max-w-7xl gap-14 px-6 py-24 lg:grid-cols-[0.7fr_1.3fr] lg:px-10 lg:py-32">
        <div><p className="text-xs font-bold uppercase tracking-[0.25em] text-[#b61f27]">01 / Who we are</p><h2 className="mt-6 max-w-sm font-serif text-4xl leading-tight tracking-tight sm:text-5xl">The number behind your next great deal.</h2></div>
        <div className="max-w-2xl"><p className="text-2xl leading-relaxed text-[#343b3d]">Powersport Appraisals is a dealer-to-dealer service built to help you make smarter inventory decisions, faster.</p><p className="mt-7 leading-7 text-[#6c7474]">When a customer brings in a motorcycle, ATV, UTV, trike, or snowmobile, timing matters. We turn the unknown into a clear wholesale figure — so your team can take the next step with confidence.</p><div className="mt-12 grid grid-cols-2 gap-8 border-t border-[#d9d6d0] pt-7 sm:grid-cols-3"><div><strong className="font-serif text-4xl text-[#b61f27]">15+</strong><p className="mt-2 text-xs uppercase tracking-widest text-[#687070]">Years of expertise</p></div><div><strong className="font-serif text-4xl text-[#b61f27]">24h</strong><p className="mt-2 text-xs uppercase tracking-widest text-[#687070]">Purchase follow-up</p></div><div><strong className="font-serif text-4xl text-[#b61f27]">100%</strong><p className="mt-2 text-xs uppercase tracking-widest text-[#687070]">Dealer focused</p></div></div></div>
      </section>

      <section id="why" className="bg-[#172021] px-6 py-24 text-white lg:px-10 lg:py-28"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-8 border-b border-white/15 pb-12 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-[#f1c15c]">02 / Why dealers choose us</p><h2 className="mt-5 max-w-xl font-serif text-4xl leading-tight sm:text-5xl">Clarity when the showroom is moving fast.</h2></div><p className="max-w-xs text-sm leading-6 text-white/50">A better appraisal is more than a number. It is a faster, more confident way to do business.</p></div><div className="grid divide-y divide-white/15 md:grid-cols-3 md:divide-x md:divide-y-0">{benefits.map(([number, title, description]) => <div className="py-8 md:px-8 md:first:pl-0 md:last:pr-0" key={number}><span className="text-xs text-[#d7a23b]">{number}</span><h3 className="mt-12 text-xl font-semibold">{title}</h3><p className="mt-4 max-w-xs text-sm leading-6 text-white/55">{description}</p></div>)}</div></div></section>

      <section id="vehicles" className="mx-auto max-w-7xl px-6 py-24 lg:px-10 lg:py-32"><div className="mb-14 flex flex-col justify-between gap-6 sm:flex-row sm:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-[#b61f27]">03 / What we appraise</p><h2 className="mt-5 font-serif text-4xl sm:text-5xl">Every ride has a story.<br /><span className="text-[#89908f]">We know how to read it.</span></h2></div><a href="#contact" className="text-xs font-bold uppercase tracking-[0.16em] text-[#b61f27]">See all vehicle types →</a></div><div className="grid gap-4 md:grid-cols-3">{vehicleTypes.map((vehicle) => <article className="group min-h-[260px] border border-[#d9d6d0] bg-white p-7 transition hover:-translate-y-1 hover:border-[#b61f27] hover:shadow-xl" key={vehicle.number}><div className="flex items-start justify-between"><span className="text-xs font-bold text-[#b61f27]">{vehicle.number}</span><span className="text-2xl text-[#d7a23b] transition group-hover:translate-x-1">↗</span></div><h3 className="mt-24 font-serif text-3xl">{vehicle.title}</h3><p className="mt-3 text-sm leading-6 text-[#727a79]">{vehicle.description}</p></article>)}</div></section>

      <section id="contact" className="relative overflow-hidden bg-[#b61f27] px-6 py-24 text-white lg:px-10 lg:py-28"><div className="absolute -right-8 -top-16 font-serif text-[240px] leading-none text-white/5">PA</div><div className="relative mx-auto flex max-w-7xl flex-col justify-between gap-10 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[0.25em] text-[#f5d37f]">Ready when you are</p><h2 className="mt-5 max-w-2xl font-serif text-5xl leading-none sm:text-7xl">Put a number<br /><em className="font-normal text-[#f5d37f]">on the next move.</em></h2></div><a href="tel:3054842442" className="inline-flex shrink-0 items-center gap-5 border border-white/40 px-7 py-5 text-xs font-bold uppercase tracking-[0.16em] transition hover:bg-white hover:text-[#b61f27]">Call 305.484.2442 <span className="text-xl">→</span></a></div></section>
      <footer className="bg-[#111516] px-6 py-8 text-white/45 lg:px-10"><div className="mx-auto flex max-w-7xl flex-col justify-between gap-4 text-[10px] uppercase tracking-[0.2em] sm:flex-row"><span>© 2025 Powersport Appraisals</span><span>Wholesale values. Real commitment.</span></div></footer>
    </main>
  )
}
